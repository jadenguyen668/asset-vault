		...
		var SkeletonRenderer = (function () {
			...
			SkeletonRenderer.prototype.drawImages = function (skeleton) {
				...
				for (var i = 0, n = drawOrder.length; i < n; i++) {
					...
					ctx.scale(1, -1);
					ctx.translate(-w / 2, -h / 2);
					// Always set globalAlpha (fix: prevents stale alpha leaking between slots)
			...
			SkeletonRenderer.prototype.drawTriangles = function (skeleton) {
				...
				for (var i = 0, n = drawOrder.length; i < n; i++) {
					...
					if (attachment instanceof spine.ClippingAttachment) {
						...
						var clipVertices = spine.Utils.setArraySize([], nn);
						clip.computeWorldVertices(slot, 0, nn, clipVertices, 0, 2);
						ctx.save();
						ctx.beginPath();
						ctx.moveTo(clipVertices[0], clipVertices[1]);
						for (var ci = 2; ci < nn; ci += 2) {
							ctx.lineTo(clipVertices[ci], clipVertices[ci + 1]);
						}
						ctx.closePath();
						ctx.clip();
						clipActive = true;
					...
					if (clipActive && attachment == null) { /* non-visual slot */ }
					if (attachment instanceof spine.RegionAttachment) {
					...
					if (texture != null) {
						...
						if (typeof slotBlendMode === 'object' || typeof slotBlendMode === 'string') {
							// Spine 3.7 may use enum: spine.BlendMode.Additive etc.
						...
						// DEBUG: log blend mode once per type
						if (!this._blendLogged) this._blendLogged = {};
						if (!this._blendLogged[slot.data.name]) {
							this._blendLogged[slot.data.name] = true;
							var msg = '[BLEND] slot:"' + slot.data.name + '" raw:' + slotBlendMode + ' type:' + typeof slotBlendMode + ' resolved:' + blendModeNum;
							console.log(msg);
						...
						if (blendModeNum !== 0) {
							// NON-NORMAL BLEND: offscreen canvas approach
							// Draw all triangles to offscreen with source-over (no seam amplification),
							// then composite entire result to main canvas with blend mode
							var mainCanvas = ctx.canvas;
							if (!this._offCanvas) {
								this._offCanvas = document.createElement('canvas');
								this._offCtx = this._offCanvas.getContext('2d');
							}
							var offCanvas = this._offCanvas;
							var offCtx = this._offCtx;
							if (offCanvas.width !== mainCanvas.width || offCanvas.height !== mainCanvas.height) {
								offCanvas.width = mainCanvas.width;
								offCanvas.height = mainCanvas.height;
							}
							// Clear offscreen with identity transform
							offCtx.setTransform(1, 0, 0, 1, 0, 0);
							offCtx.globalCompositeOperation = "source-over";
							offCtx.clearRect(0, 0, offCanvas.width, offCanvas.height);
							// Copy main canvas transform to offscreen
							offCtx.setTransform(ctx.getTransform());
							offCtx.globalAlpha = 1;
							// Draw all triangles to offscreen with tinted texture (source-over = no seam issues)
							var drawTex = texture;
							if (color.r < 0.99 || color.g < 0.99 || color.b < 0.99) {
								try { drawTex = getTintedImage(texture, color.r, color.g, color.b); } catch (e) { drawTex = texture; }
							}
							for (var j = 0; j < triangles.length; j += 3) {
								...
								this.drawTriangleToCtx(offCtx, drawTex, x0, y0, u0, v0, x1, y1, u1, v1, x2, y2, u2, v2);
							}
							// Composite offscreen to main canvas with blend mode
							ctx.save();
							...
							// NORMAL BLEND: direct rendering with triangle expansion
							ctx.globalAlpha = color.a;
							...
							for (var j = 0; j < triangles.length; j += 3) {
								...
								var x1 = vertices[t2], y1 = vertices[t2 + 1], u1 = vertices[t2 + 6], v1 = vertices[t2 + 7];
								var x2 = vertices[t3], y2 = vertices[t3 + 1], u2 = vertices[t3 + 6], v2 = vertices[t3 + 7];
								this.drawTriangleToCtx(ctx, texture, x0, y0, u0, v0, x1, y1, u1, v1, x2, y2, u2, v2);
							}
						}
					}
				}
			...
			// Cache tinted atlas textures per-color
			var _tintCache = {};
			function getTintedImage(img, r, g, b) {
				if (!img || !img.width || !img.height) return img;
				var key = Math.round(r * 255) + ',' + Math.round(g * 255) + ',' + Math.round(b * 255);
				var cached = _tintCache[key];
				if (cached && cached.src === img && cached.w === img.width && cached.h === img.height) return cached.canvas;
				...
				tctx.drawImage(img, 0, 0);
				_tintCache[key] = { canvas: tc, src: img, w: img.width, h: img.height };
				return tc;
			}
			// Draw a single triangle to any canvas context
			SkeletonRenderer.prototype.drawTriangleToCtx = function (targetCtx, img, x0, y0, u0, v0, x1, y1, u1, v1, x2, y2, u2, v2) {
				u0 *= img.width; v0 *= img.height;
				u1 *= img.width; v1 *= img.height;
				u2 *= img.width; v2 *= img.height;
				// Expand clip path slightly to reduce seam lines
				var cx = (x0 + x1 + x2) / 3, cy = (y0 + y1 + y2) / 3;
				var expand = 0.5, dx, dy, len;
				var ex0 = x0, ey0 = y0, ex1 = x1, ey1 = y1, ex2 = x2, ey2 = y2;
				dx = x0 - cx; dy = y0 - cy; len = Math.sqrt(dx * dx + dy * dy);
				if (len > 0) { ex0 += dx / len * expand; ey0 += dy / len * expand; }
				dx = x1 - cx; dy = y1 - cy; len = Math.sqrt(dx * dx + dy * dy);
				if (len > 0) { ex1 += dx / len * expand; ey1 += dy / len * expand; }
				dx = x2 - cx; dy = y2 - cy; len = Math.sqrt(dx * dx + dy * dy);
				if (len > 0) { ex2 += dx / len * expand; ey2 += dy / len * expand; }
				targetCtx.beginPath();
				targetCtx.moveTo(ex0, ey0);
				targetCtx.lineTo(ex1, ey1);
				targetCtx.lineTo(ex2, ey2);
				targetCtx.closePath();
